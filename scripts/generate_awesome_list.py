...
_CATEGORY_MARKERS = {"legacy": "🕰️", "experimental": "⚗️"}
...
def render_repos(repos: list[dict[str, Any]]) -> list[str]:
    ...
    seen: set[str] = set()
    entries: list[tuple[str, str, str, str]] = []  # (sort_key, link, desc, markers)
    for repo in repos:
        url = repo["repo"]
        hooks = repo.get("hooks")
        if url not in seen and url != "meta" and hooks:
            seen.add(url)
            link, desc = repo_link(url)
            sort_key = url.rstrip("/").split("/")[-1].lower()
            entries.append((sort_key, link, desc, category_markers(hooks)))
    return [
        f"- {markers + ' ' if markers else ''}{link}" + (f" — {desc}" if desc else "")
        for _, link, desc, markers in sorted(entries)
    ]
...
def build_awesome_list(languages: list[str], frameworks: list[str]) -> str:
    ...
    header = [
        ...
        "",
        (
            "_Auto-generated by [pc-init](https://github.com/cleder/gpc-init) presets"
            " — to add or update a hook, open a PR there rather than editing this file"
            " directly._"
        ),
        "",
        (
            "Hooks are included by default unless marked otherwise: "
            "🕰️ legacy (superseded by a newer hook, still usable, opt in with "
            "`--profile legacy`) or ⚗️ experimental (new, not yet proven, opt in "

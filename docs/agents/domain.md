...
- **`.claude/docs/adr/`** — read ADRs that touch the area you're about to work in. (Note: this repo uses `.claude/docs/adr/`, not the more common `docs/adr/`.)
...
├── CONTEXT.md
├── .claude/docs/adr/
│   └── 0001-hook-profile-categories.md
└── gpc_init/
```
...
> _Contradicts ADR-0001 (hook profile categories) — but worth reopening because…_
